...
on 2,483 REAL human attacks (HackAPrompt) — a source neither detector trained on — at MATCHED
clean-FPR. HackAPrompt is positive-only (all real attacks), so we compare recall at fixed FPR set on
...
Shipped config (0.5B concat L13-15). Cached training embeddings; embed HackAPrompt fresh. Seed 42.
"""
...
# HackAPrompt real attacks
hp=[r["text"] for r in json.load(open("research/datasets/pi_hackaprompt.json")) if r.get("text")]
...
EMBhp=embed_all(hp,"hackaprompt2483")
...
json.dump(res,open(f"{OUTDIR}/pi_hackaprompt_recall.json","w"),indent=2)
print("\n=== REAL-HUMAN recall on HackAPrompt (trained on other sources) ===",flush=True)
